...
def tampilkan_data(lihat):
    ...
    for index,i in enumerate(initiate_dict_iterator(load_dict())):
        if i['role'] == lihat or lihat == 'all':
            ...
            print('Nama: ', i['nama'])
            if i['role'] != 'teacher':
                uh_list_temp = [i['nilai_uh_1'], i['nilai_uh_2'], i['nilai_uh_3']]
                avg_uh = lambda nilai1, nilai2, nilai3: (nilai1 + nilai2 + nilai3) / len(uh_list_temp)
                uh_list_temp = list(filter(no_zero_filter, uh_list_temp))
                for index, nilai in enumerate(uh_list_temp):
                    print('Nilai UH '+str(index)+": "+map_nilai(int(nilai)))
                print('Nilai Rata-rata UH: ', reduce(lambda x,y : x+y, uh_list_temp) / len(uh_list_temp))
