...
int main(){
	...
	passlen,userlen,usercmp,passcmp,kodecmp,noUrut,pil,spcchchk;
	char regpasschk[20],userString[20],passString[20],kodeString[9],kodeStringTemp[9],
	userName[besarTabel][20],password[besarTabel][20],kodeUser[besarTabel][8],
	...
	{case 1:
		...
		for(i=0;regUserName[i]!=0;i++){
			if(regUserName[i]!='.'){
			spcchchk=ispunct(regUserName[i]);}
			if(spcchchk!=0){
			spcch++;}
		}if(spcch>0){
		...
		for(i=0;regPassword[i]==0;i++){
			spcchchk=ispunct(regPassword[i]);
			if(spcchchk==0){
			spcch++;}else{
			spcch=0;
			break;}
