...
#include <conio.h>
int main(){
	int lcase=0,ucase=0,numer=0,spcch=0,i,j,k,passlen,userlen,usercmp,passcmp,noUrut;
	char reguserchk[20],userString[20],passString[20],kodeString[20], 
	userName[50][20], password[50][20], kodeUser[50][5], regUserName[20], regPassword[20],
	inputUsername[20], inputPassword[20];
	int pil;
	for(j=0;j<=0;j++){
		for(i=0;i<=50;i++){
			userName[j][i]='\0';
	...
	{case 1:
		...
		goto reg1;}
		spcch=0; passlen=strlen(regPassword);
		if(passlen<8){
	...
	for(k=0;k<=0;k++){
		for(i=0;i<=50;i++){
					for(k=0;k<=20;k++){userString[k]=userName[i][k];}
					usercmp=strcmp(userString,inputUsername);
					if(usercmp==0){
						printf("input password: ");
						scanf("%s",inputPassword);
						for(k=0;k<=20;k++){passString[k]=password[i][k];}
	...
	case 3:
		...
		for(j=0;j<=50;j++){
			if(userName[j][1]){
				printf("\n\n==%d==\n",(j+1));
				printf(" Username : ");
				for(i=0;userName[j][i]!=0;i++){printf("%c",userName[j][i]);}
				printf("\nKode User : ");
